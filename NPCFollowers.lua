...
-- 魔法大師=19，兵器大師=16
function setNpcType(i, t)
	local id = getNpcID(i)
	if (id > 0) then
		print("NPC ID:", id)
		print("type changed to:", t)
		Game.NPC[id].Profession = t
	end
end

function getNpcFace(i)
	local id = getNpcID(i)
	if (id > 0) then
		print("NPC ID:", id)
		print("  face:", Game.NPC[id].Pic)
	end
end

-- 部分較為好看的臉：1231、1244、193、198、213、267?、822、844、851、852（翡翠島琴姐）、853（翡翠島嚮導女）。七代精靈臉641-671
function setNpcFace(i, face)
	local id = getNpcID(i)
	if (id > 0) then
		print("NPC ID:", id)
		print("face changed to:", face)
		Game.NPC[id].Pic = face
	end
end
--1231,198,851,853
